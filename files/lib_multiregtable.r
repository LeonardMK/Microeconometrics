    ...
    for (m in 1:(cols-1)){
        for (v in 1:rows){
            if (varnames[v] %in% names(models[[m]]$coefficients)) {
                ...
                rettab[v*2 - 1, m+1] <- paste(roundsig(models[[m]]$coefficients[varnames[v]], roundparam), "$^{", multiregtable_sigsymbol(t.ratio), "}$", sep="")
                rettab[v*2, m+1] <- paste("{\\scriptsize (", roundsig(sqrt(thevar[varnames[v],varnames[v]]), roundparam), ")}",  sep="")
            }
            else {
                rettab[v*2 - 1, m+1] <- c("")
                rettab[v*2, m+1] <- c("")
